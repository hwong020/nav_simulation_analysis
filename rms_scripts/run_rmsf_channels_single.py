...
def plot_channel(channel_dir: Path, output_dir: Path) -> None:
    ...
    residues, trial_values = collect_trials(channel_dir / "rmsf_single_lig")
    ...
    style_axes(ax)
    ax.set_title(f"{channel_name} (Bound)", fontsize=TITLE_FONT_SIZE, pad=10)
...
def main() -> None:
    ...
    for channel_dir in sorted(src_root.glob("nav1-*")):
        rmsf_dir = channel_dir / "rmsf_single_lig"
        if not rmsf_dir.exists():
            print(f"Skipping {channel_dir.name}: no rmsf_single_lig directory")
            continue
